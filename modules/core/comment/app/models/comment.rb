class Comment < ActiveRecord::Base
  ...
  validates_presence_of :model_type, :model_id, :body
  ...
  def validate
    if !ActiveRecord::Acts::Commentable.model_types.include?(model_type)
      errors.add_to_base(I18n.t('common.hack_you'))
    else
      klass = model_type.constantize
      errors.add_to_base(I18n.t('comment.model_deleted')) unless klass.exists?(model_id)
    end
  end
